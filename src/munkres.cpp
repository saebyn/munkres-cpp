...
void minimize_along_direction (Matrix<double> & matrix, const bool over_columns)
{
...
bool Munkres::find_uncovered_in_matrix (const double item, size_t & row, size_t & col) const
{
    const size_t rows = matrix.rows (),
                 columns = matrix.columns ();
...
bool Munkres::pair_in_list (const std::pair<size_t,size_t> & needle, const std::list<std::pair<size_t,size_t>> & haystack)
{
